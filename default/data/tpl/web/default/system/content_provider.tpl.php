...
<ul class="nav nav-tabs">
	<li<?php  if($do == 'finance_info' || $do == 'account_list' || $do == 'display' || $do == 'register_flow' || $do == 'flow_control') { ?> class="active"<?php  } ?>>
		<a href="<?php  echo url('system/content_provider/account_list');?>">流量主</a>
	</li>
	<li<?php  if($do == 'content_provider') { ?> class="active"<?php  } ?>><a href="<?php  echo url('system/content_provider/content_provider');?>">广告主</a></li>
</ul>
<style>
	.nav .nav-tabs-flow {
...
<div class="clearfix">
	...
	<ul class="nav nav-tabs-flow">
		<li<?php  if($do == 'account_list' || $do == 'display' || $do == 'register_flow' || $do == 'flow_control') { ?> class="active"<?php  } ?>>
			...
			<a href="<?php  echo url('system/content_provider/account_list');?>">流量管理</a>
			<?php  } else if($flow_master_info['status'] != 4) { ?>
			<a href="<?php  echo url('system/content_provider/display');?>">流量管理</a>
			<?php  } ?>
		</li>
		<?php  if(!empty($commission_show) || $flow_master_info['status'] == 4) { ?>
		<li<?php  if($do == 'finance_info') { ?> class="active"<?php  } ?>><a href="<?php  echo url('system/content_provider/finance_info');?>">财务信息</a></li>
		<?php  } ?>
	</ul>
	<?php  } ?>
	<?php  if($do == 'display') { ?>
		...
		<div class="panel-body">
			...
			<div class="col-xs-5">
				<span class="pull-right">
					<a class="btn btn-primary" href="<?php  echo url('system/content_provider/register_flow')?>">申请流量主</a>
				</span>
			</div>
			<?php  } ?>
		</div>
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			<form action="" method="post"  class="form-horizontal" role="form" enctype="multipart/form-data" id="form1">
				...
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label">身份证上传</label>
					<div class="col-sm-9 col-xs-12">
				...
				<?php  } ?>
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label">营业执照上传</label>
					<div class="col-sm-9 col-xs-12">
		...
		<div class="panel panel-info">
			...
			<div class="panel-body">
				<form action="./index.php" method="get" role="form" class="form-horizontal">
					...
					<input type="hidden" name="a" value="content_provider">
		...
		<div class="panel panel-default">
			...
			<div class="table-responsive panel-body">
				<table class="table table-hover">
					<thead>
						<tr>
							<th class="row-hover">公众号<i></i></th>
							<th style="width:300px;">标签<i></i></th>
							<?php  if(!empty($commission_show)) { ?>
							<th style="width:100px;">累计金额<i></i></th>
							<?php  } ?>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<a href="<?php  echo url('system/content_provider/flow_control', array('uniacid' => $account['uniacid']))?>">
									<?php  if(IMS_FAMILY == 'v' ||$uni['flow_setting_enable'] == 2) { ?>
									设置
									<?php  } else if($uni['flow_setting_enable'] == 1) { ?>
									开通广告联盟
				...
				<table class="table table-hover">
					<thead>
						<tr>
							...
							<th style="width:300px;">标签<i></i></th>
							<?php  if(!empty($commission_show)) { ?>
							<th style="width:150px;">累计金额<i></i></th>
							<?php  } ?>
					...
					<tbody>
						<tr>
							...
							<?php  if(IMS_FAMILY != 'v') { ?>
								...
								<a href="#ad-tags" data-toggle="modal" class="btn btn-primary">开通</a>
								<?php  } else if($current_uniaccount['enable'] == 2) { ?>
								<a href="javascript:;" class="btn btn-danger js-stop-provider" data-uniacid="<?php  echo $_GPC['uniacid'];?>">停止</a>
								<a href="#ad-tags" data-toggle="modal" class="btn btn-primary">设置</a>
								<?php  } ?>
							<?php  } else { ?>
								<a href="#ad-tags" data-toggle="modal" class="btn btn-primary">设置</a>
							<?php  } ?>
						...
						<table class="table tb" style="min-width:568px;">
							...
							<?php  if(!empty($tag_list)) { ?>
								<?php  if(is_array($tag_list)) { foreach($tag_list as $tags) { ?>
									...
									<tr>
										<td class="text-center"><input type="checkbox" id="chk_tag_<?php  echo $key;?>" name="tagids[]" data-name="<?php  echo $items;?>" value="<?php  echo $key;?>" <?php  if(!empty($current_tags[$key])) { ?> checked<?php  } ?>></td>
										<td><label for="chk_tag_<?php  echo $key;?>" style="font-weight:normal;" class="name"><?php  echo $items;?></label></td>
									</tr>
									<?php  } } ?>
								<?php  } } ?>
							<?php  } ?>
		...
		<script>
			...
			$(document).on('click', '.js-type-setting', function() {
				...
				for (var i in cloud_module_support_str[module_name].ad_types) {
					...
					html += '<td><label for="chk_type_'+ i +'" style="font-weight:normal;" class="name">' +
				...
				$.post("<?php  echo url('system/content_provider/ad_type_get')?>", {'module_name' : module_name, 'uniacid' : uniacid}, function(data) {
					data = $.parseJSON(data);
			...
			$('.js-select-tags').click(function(){
				...
				if (tagids.length == 0) {
					...
					$.post("<?php  echo url('system/content_provider/flow_control')?>", {'type' : 'tags', 'tagids' : tagids, 'enable' : 2, 'uniacid' : uniacid}, function(data) {
						data = $.parseJSON(data);
						if (data.message.errno == 0) {
			...
			$('.js-select-types').click(function(){
				...
				$.post("<?php  echo url('system/content_provider/flow_control')?>", {'type' : 'types', 'typeids' : typeids, 'enable' : 2, 'module_name' : module_name, 'uniacid' : uniacid}, function(data) {
					data = $.parseJSON(data);
					if (data.message.errno == 0) {
						util.message(data.message.message, data.redirect, 'success');
					} else {
			...
			$(document).on('click', '.js-stop-provider', function() {
				var uniacid = $(this).data('uniacid');
				$.post("<?php  echo url('system/content_provider/flow_control')?>", {'type' : 'tags', 'enable' : 1, 'uniacid' : uniacid}, function(data) {
					data = $.parseJSON(data);
					if (data.message.errno == 0) {
						util.message(data.message.message, data.redirect, 'success');
					} else {
	...
	<nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin: 10px -1em 1em 0px;">
		<div class="navbar-header">
			<a class="navbar-brand" href="<?php  echo url('system/content_provider/finance_info')?>">数据统计</a>
			<ul class="nav navbar-nav navbar-right">
				<li <?php  if($_GPC['type'] == 1) { ?>class="active"<?php  } ?>><a href="<?php  echo url('system/content_provider/finance_info', array('type' => '1'))?>">今日</a></li>
				<li <?php  if($_GPC['type'] == 2) { ?>class="active"<?php  } ?>><a href="<?php  echo url('system/content_provider/finance_info', array('type' => '2'))?>">昨日</a></li>
	...
	<?php  if($do == 'content_provider') { ?>
		<iframe src="<?php  echo $iframe;?>" marginheight="0" marginwidth="0" frameborder="0" width="100%" style="height:2000px;" scrolling="no" allowTransparency="true"></iframe>
	<?php  } ?>
