...
//        pdo_update($this->table_stores, array("is_show" => $_GPC['store_status']), array('id' => $storeid, 'weid' => $weid), 'AND');
        ...
        if (empty($_GPC['store_status']) && !empty($people)) {
            ...
            $BossMessage['diningPrice'] = pdo_fetchall('SELECT paytype,sum(totalprice) as total,count(*) as num ,dining_mode as diningType FROM ' . tablename($this->table_order) . " WHERE weid = :weid AND storeid = :storeid AND status=3 AND ispay=1 $conditions  group by dining_mode", array(':weid' => $this->_weid, ':storeid' => $storeid));
            $BossMessage['payPrice'] = pdo_fetchall('SELECT paytype,sum(totalprice) as total,count(*) as num  FROM ' . tablename($this->table_order) . " WHERE weid = :weid AND storeid = :storeid AND status=3 AND ispay=1 $conditions  GROUP BY paytype ORDER BY paytype ASC", array(':weid' => $this->_weid, ':storeid' => $storeid));
            $BossMessage['totalNum'] = pdo_fetch("SELECT sum(counts) as peopleNum,sum(totalprice) as totalPrice,sum(totalprice)/sum(counts) as avgPrice ,count(1) as totalNum FROM " . tablename($this->table_order) . " WHERE storeid=:storeid and weid=:weid AND status=3 AND ispay=1 $conditions", array(":storeid" => $storeid, ":weid" => $this->_weid));
