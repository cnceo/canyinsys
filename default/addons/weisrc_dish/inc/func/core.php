        ...
        if ($_W['role'] == 'operator') {
            ...
            $navemenu[0] = array(
                ...
                'items' => array(
                    ...
                    2 => $this->createMainMenu('门店类型 ', $do, 'type', ''),                  
                    3 => $this->createMainMenu('会员管理 ', $do, 'allfans', ''),
                    4 => $this->createMainMenu('账号管理 ', $do, 'account', ''),
                    5 => $this->createMainMenu('系统设置 ', $do, 'setting', ''),
                )
            );
            ...
            $navemenu[1] = array(
                ...
                'items' => array(
                    0 => $this->createCoverMenu('首页 ', 'cover', 'index', ''),
                    ...
                    4 => $this->createCoverMenu('商家订单管理 ', 'cover', 'adminorder', ''),
                    5 => $this->createCoverMenu('配送中心入口 ', 'cover', 'deliveryorder', ''),
                )
            );
        }
        return $navemenu;
...
//        $cur_color = ' style="color:#d9534f;" ';
        ...
        $navemenu[] = array(
            ...
            'items' => array(
                ...
                0 => $this->createSubMenu('门店信息 ', $do, 'stores2', 'fa-angle-right', $cur_color, $storeid),
                1 => $this->createSubMenu('订单管理 ', $do, 'order', 'fa-angle-right', $cur_color, $storeid),
                2 => $this->createSubMenu('会员管理 ', $do, 'fans', 'fa-angle-right', $cur_color, $storeid),
                3 => $this->createSubMenu('员工管理 ', $do, 'coreuser', 'fa-angle-right', $cur_color, $storeid),
            ),
            'icon' => 'fa fa-user-md'
        );
        $cur_color = '#8d8d8d';
...
//        $filename = empty($filename) ? date("YmdHis") . '' . random(10) : date("YmdHis") . '' . random(istrlen($filename));
    ...
    {
        $datas = pdo_fetchall("SELECT * FROM " . tablename($this->table_ad) . " WHERE uniacid = :uniacid AND position=2
        AND status=1 AND :time >starttime AND :time < endtime  ORDER BY displayorder DESC,id DESC LIMIT 6", array(':uniacid' => $this->_weid, ':time' => TIMESTAMP));
        return $datas;
    }
