...
if ($op == 'acceptorder') {
    ...
    if ($delivery_order_max != 0) { //限制配送量
        $delivery_order_total = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_order) . " WHERE weid=:weid AND delivery_status=1 AND delivery_id=:delivery_id ", array(':delivery_id' => $deliveryuser['id'], ':weid' => $weid));
