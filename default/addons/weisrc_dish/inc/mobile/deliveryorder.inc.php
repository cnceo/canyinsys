...
$strwhere = " WHERE weid = '{$weid}' AND dining_mode=2 ";
//最近配送点
...
if ($op == 0) {
//    $limittime = strtotime("+5 minute");
    $strwhere .= " AND delivery_status=0 AND delivery_id=0 ";
//    $limittime = intval($setting['delivery_auto_time']);
//    if ($limittime != 0) {
//        $strwhere .= "  AND unix_timestamp(now())-dateline>{$limittime} ";
//    }
} else {
...
//todayprice
$today_order_price = pdo_fetchcolumn("SELECT sum(delivery_money) FROM " . tablename($this->table_order) . " WHERE weid=:weid AND delivery_id=:deliveryid AND
dateline>:time AND status=3 ", array(':weid' => $this->_weid, ':deliveryid' => $deliveryid, ':time' => $zero_time));
$today_order_price = sprintf('%.2f', $today_order_price);

//ordercount
$today_order_count = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename($this->table_order) . " WHERE weid=:weid AND delivery_id=:deliveryid AND dateline>:time", array(':weid' => $this->_weid, ':deliveryid' => $deliveryid, ':time' => $zero_time));
