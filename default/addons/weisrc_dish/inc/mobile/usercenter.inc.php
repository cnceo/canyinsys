...
$authurl = $_W['siteroot'] . 'app/' . $this->createMobileUrl($method, array('agentid' => $agentid), true) . '&authkey=1';
$url = $_W['siteroot'] . 'app/' . $this->createMobileUrl($method, array('agentid' => $agentid), true);
...
if ($agentid != 0) {
    ...
    if ($setting['commission_mode'] == 2) { //代理商模式
        if ($agent['is_commission'] != 2) {//用户不是代理商重新查找
            $agent = pdo_fetch("SELECT * FROM " . tablename($this->table_fans) . " WHERE id=:id AND weid=:weid LIMIT 1", array(':id' => $agent['agentid'], ':weid' => $weid));
    ...
    if (!empty($agent['agentid'])) {
        ...
        $agent2 = pdo_fetch("SELECT * FROM " . tablename($this->table_fans) . " WHERE id=:id AND weid=:weid LIMIT 1", array(':id' => $agentid2, ':weid' => $weid));
