...
//老板通知
    ...
    public function doMobilePay()
    {
        ...
        if (IMS_VERSION != '0.7') {
            ...
            //支付,取消,完成
            if ($order['status'] == 3) {
                message('抱歉，您的订单已经完成交易！', referer(), 'error');
            }
...
EOF;
    ...
    public function getNearDeliveryArea($lat, $lng)
    {
        ...
        $area = pdo_fetch("SELECT *,(lat-:lat) * (lat-:lat) + (lng-:lng) * (lng-:lng) as dist FROM " . tablename
            ($this->table_deliveryarea) . " {$strwhere} {$strorder}
 LIMIT 1", array(':weid' => $weid, ':lat' => $lat, ':lng' => $lng));
...
//                            $this->setFansCoin($agent['from_user'], $commission1_price, "单号{$order['ordersn']}一级佣金奖励");
                    ...
                    if ($setting['commission_level'] > 1) {
                        if ($agent['agentid'] > 0) {
                            ...
                            if ($is_commission == 1) {
                                ...
                                pdo_insert($this->table_commission, $data);
                                if ($setting['commission_settlement'] == 1) {
//                                    $this->setFansCoin($agent2['from_user'], $commission2_price, "单号{$order['ordersn']}二级佣金奖励");
...
DESC LIMIT 1", array(':weid' => $this->_weid, ':goodsid' => $item['goodsid']));
        ...
        $touser = $_W['user']['username'] . '&nbsp;退菜：' . $goods['title'] . "*1" . ",";
        $this->addOrderLog($orderid, $touser, 2, 2, 1);

        $totalprice = floatval($order['totalprice']) - $goodsprice;
        pdo_update($this->table_order, array('totalprice' => $totalprice), array('weid' => $this->_weid, 'id' => $orderid));
...
DESC LIMIT 1", array(':tid' => $orderid, ':uniacid' => $this->_weid));
    ...
    {
        //自己设置的上传文件存放路径
        $filePath = IA_ROOT . '/addons/weisrc_dish/upload/';
    ...
    function upload_goods($strs, $time, $array)
    {
        ...
        $goods = pdo_fetch("SELECT * FROM " . tablename($this->table_goods) . " WHERE title=:title AND weid=:weid AND storeid=:storeid", array(':title' => $strs[1], ':weid' => $weid, ':storeid' => $storeid));
...
print_usr=:print_usr) AND storeid = :storeid {$condition} ORDER BY id DESC limit 1", array(':storeid' => $storeid, ':print_usr' => $usr));
        ...
        $storeid = $order['storeid'];
        $store = $this->getStoreById($storeid);
        $paytype = array('0' => '线下付款', '1' => '余额支付', '2' => '微信支付', '3' => $order['dining_mode'] == 1 ? '下单' : '货到付款',
            '4' => '支付宝');
